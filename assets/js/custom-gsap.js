...
(function ($) {
  ...
  // Robust Navigation & Offcanvas Anchor Click Handler
  $(document).on("click", 'a[href^="#"], .nav-section-link, .footer-quick-link', function (e) {
    ...
    if (href && href !== "#" && href.length > 1) {
      ...
      if (targetElem) {
        ...
        // 1. Immediately close offcanvas overlay drawer
        $(".tw-offcanvas-2-area").removeClass("opened");
        $(".side-overlay").removeClass("opened");
        $(".body-overlay").removeClass("opened");

        // 2. Perform smooth scroll via GSAP ScrollSmoother or Fallback
        setTimeout(function() {
          if (typeof smoother !== "undefined" && smoother) {
            smoother.scrollTo(href, true);
          } else {
            var targetPos = $(href).offset().top - 60;
            $("html, body").stop().animate({ scrollTop: targetPos }, 700);
          }
        }, 150);
      }
    }
  ...
  // Dedicated Footer Quick Link Smooth Scroll Handler
  $(document).on("click", ".footer-quick-link, .footer-three-back-to-top", function (e) {
    var href = $(this).attr("href");
    if (href && href !== "#" && href.length > 1) {
      var target = $(href);
      if (target.length) {
        e.preventDefault();
        if (typeof smoother !== "undefined" && smoother) {
          smoother.scrollTo(target[0], true, "top 40px");
        } else {
